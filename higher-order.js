...
//Code Here
let postTaxPrices = prices.map((val) => val = (val*.07) + val) // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((a,e) => a + e) //  = populations.reduce(/* Provide Your Callback Here */)
...
let ordersTotal = orders.map(val => {return val.price = val.price + (val.price * val.tax)})
...
let bobsTotal = purchases.filter(val => val.owner === "Bob").map(val => val.price).reduce((a, e) => a + e)
